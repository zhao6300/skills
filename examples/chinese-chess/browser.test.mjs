...
test("AI opponent can answer a human move", async () => {
  const server = createServer(async (request, response) => {
    const path = request.url === "/" ? "/index.html" : request.url ?? "/index.html";
    try {
      response.setHeader("Content-Type", types[extname(path)] ?? "application/octet-stream");
  ...
  try {
    ...
    page.on("pageerror", (error) => errors.push(error.message));
    await page.goto(`http://127.0.0.1:${server.address().port}/`);
    await page.click('[data-mode="human-ai"]');
    assert.equal(await page.locator('[data-mode="human-ai"][aria-pressed="true"]').count(), 1);
    await page.click('[data-x="0"][data-y="9"]');
    await page.click('[data-x="0"][data-y="8"]');
