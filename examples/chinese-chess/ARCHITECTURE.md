...
| AI policy | pure `ai.js` | bounded search memory: TT, killers, history | board -> legal move + score + one-line explanation | iterative alpha-beta with TT/MVV-LVA move ordering; recommend but do not move unless accepted | return `null` and hide card; rules remain engine-owned | `ai.test.mjs` | hide suggestion |
